export const SLIDER_DEBOUNCE_TIME = 50;

export const MAP_SIZE_X = 1920;
export const MAP_SIZE_Y = 1080;

export const MIN_ZOOM = 1;
export const MAX_ZOOM = 5;

export const ZOOM_THRESHOLD = 10;
export const ZOOM_SCALE_FACTOR = 0.005;
export const CAMERA_UPDATE_INTERVAL = 50;

export const YEARS_MIN = 1950;
export const YEARS_MAX = 2023;

export const MOVEMENT_THRESHOLD = 5;
export const MOVEMENT_SCALE_FACTOR = 0.15;

export const MESSAGE_TIMEOUT = 3000;

export const MAX_WIND_SPEED = 30;

/* just for dev stuff */
export const MOVE_DELTA = 100;
